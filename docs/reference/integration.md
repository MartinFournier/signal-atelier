...
| [Traveler's Backpack](https://modrinth.com/project/rlloIFEV) | Modest expedition storage that can travel through the death-recovery loop. | Capacity, nesting, utilities, loot, higher tiers, and extra tank upgrades are restricted; the two recipe-required 1,000 mB tanks remain. | Test capacities, tanks, nesting, graves, placement, pickup, logout, and restart. |
...
| [Xaero's World Map](https://modrinth.com/project/NcUtCpym) | Explored-terrain navigation and waypoints for remote Oritech sites. | Paired with the minimap for waypoint guidance; cave mapping, entity tracking, and teleportation remain outside pack policy. | Verify explored-only mapping, waypoint sharing, and teleport/radar state. |
| [Xaero's Minimap](https://modrinth.com/project/1bokaNcj) | Nearby terrain and in-world waypoint guidance complement the explored-terrain World Map. | Client-only; first-install defaults enable waypoints while disabling cave maps, entity radar, and cross-world teleportation. | Confirm defaults, waypoint synchronization, underground privacy, and control conflicts. |
...
| [Distant Horizons](https://modrinth.com/project/uCdwusMi) | Retains explored terrain as distant LODs for long views between remote worksites. | Client-only; capped at a 128-chunk LOD radius with distant generation disabled so unexplored terrain is not revealed. | Test transitions, underground behavior, storage growth, and frame pacing. |
| [BetterF3](https://modrinth.com/project/8shC1gFX) | Makes the dense debug overlay easier to scan during play and testing. | Client-only and backed by the existing Cloth Config dependency. | Confirm layout, Whimscape readability, configuration, and vanilla debug shortcuts. |
