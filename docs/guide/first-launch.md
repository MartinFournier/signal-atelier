...
Open **Options → Controls → Key Binds** and search for conflicts. First installs
default to `M` for the world map, `B` for the equipped backpack, `J` for the
engineering notebook, and `V` for Oritech's augment screen. Default Options
preserves later player changes.
